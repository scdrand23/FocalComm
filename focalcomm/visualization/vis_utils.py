...
from focalcomm.utils import common_utils
...
def visualize_single_sample_output_gt(pred_box_tensor,
                                    ...
                                    mode='constant'):
    """
    Visualize only bounding boxes without point clouds.
    """
    ...
    # Create list to store visualization elements (only boxes)
    visualize_elements = []
    
    # Add prediction boxes if available
    if pred_box_tensor is not None:
        # Bright red for predictions
        pred_boxes = bbx2oabb(pred_box_tensor, color=(1, 0, 0))
        visualize_elements.extend(pred_boxes)

    # Add ground truth boxes with bright blue
    gt_boxes = bbx2oabb(gt_box_tensor, color=(0, 0, 1))
    visualize_elements.extend(gt_boxes)

    # Show visualization if requested
    if show_vis:
        custom_draw_geometry_boxes_only(
            pred_boxes if pred_box_tensor is not None else [], 
            gt_boxes
        )

    # Save visualization if path provided
    if save_path:
        save_o3d_visualization_boxes_only(visualize_elements, save_path)

def custom_draw_geometry_boxes_only(pred_boxes, gt_boxes):
    """
    Custom visualization for boxes only
    """
    ...
    opt.line_width = 5.0  # Much thicker lines for visibility

    # Add geometries (only boxes)
...
def save_o3d_visualization_boxes_only(boxes, save_path):
    """
    Save visualization of only the boxes.
    """
    ...
    for box in boxes:
        # Get the box corners for OrientedBoundingBox
        box_points = np.asarray(box.get_box_points())
        
        # Ensure color is in valid RGB format
        color = np.clip(box.color, 0, 1)  # Clip color values between 0 and 1
        
        # Define the box vertices connections
        edges = [
        ...
        for edge in edges:
            ax.plot3D(
                [box_points[edge[0]][0], box_points[edge[1]][0]],
                [box_points[edge[0]][1], box_points[edge[1]][1]],
                [box_points[edge[0]][2], box_points[edge[1]][2]],
                color=color,
                linewidth=3
            )
        ...
        for face in faces:
            ...
            face_color = (*color, 0.3)  # RGB + alpha
    ...
    # Auto-set limits based on boxes
    all_points = np.vstack([np.asarray(box.get_box_points()) for box in boxes])
    margin = 5
...
def custom_draw_geometry(pcd, pred_boxes, gt_boxes):
    """
    Custom visualization with enhanced settings
    """
    vis = o3d.visualization.Visualizer()
    vis.create_window()
    ...
    opt = vis.get_render_option()
    opt.background_color = np.asarray([1, 1, 1])  # White background
    opt.point_size = 1.5  # Larger point size
...
#     """
#     Save visualization using matplotlib with enhanced bbox visibility.
#     """
#     import matplotlib.pyplot as plt
#     from mpl_toolkits.mplot3d.art3d import Poly3DCollection
...
#             # Make points smaller and more transparent
#             ax.scatter(points[:, 0], points[:, 1], points[:, 2],
#                       c=colors if len(colors) > 0 else None,
#                       s=0.5, alpha=0.3)  # Reduced point size and transparency
...
#             color = colors[0] if len(colors) > 0 else np.array([1, 0, 0])  # Default to red
            
#             # Make bboxes more visible
#             for line in lines:
#                 xs = [points[line[0]][0], points[line[1]][0]]
#                 ys = [points[line[0]][1], points[line[1]][1]]
#                 zs = [points[line[0]][2], points[line[1]][2]]
#                 ax.plot(xs, ys, zs, color=color, linewidth=2.0)  # Increased linewidth
...
#                 z = points[face, 2]
#                 verts = [list(zip(x, y, z))]
#                 pc = Poly3DCollection(verts, alpha=0.2)  # Increased transparency
#                 pc.set_facecolor(color)
#                 pc.set_edgecolor(color)
#                 ax.add_collection3d(pc)
    
#     # Adjust view angle for better bbox visibility
#     ax.view_init(elev=30, azim=45)
...
#     # Set equal aspect ratio
#     ax.set_box_aspect([1, 1, 0.5])  # Adjusted for better visibility
    
#     # Auto-set limits based on point cloud
#     for geom in element:
#         if isinstance(geom, o3d.geometry.PointCloud):
#             points = np.asarray(geom.points)
...
#             ax.set_zlim([np.min(points[:, 2]) - margin, np.max(points[:, 2]) + margin])
#             break
