...
import focalcomm.visualization.simple_plot3d.canvas_3d as canvas_3d
import focalcomm.visualization.simple_plot3d.canvas_bev as canvas_bev
...
def visualize(infer_result, pcd, pc_range, save_path, method='3d', left_hand=False):
        ...
        infer_result:
            pred_box_tensor : torch.Tensor
                (N, 8, 3) prediction.
            pred score : torch.Tensor
                (N, 2) prediction score.            
            gt_box_tensor : torch.Tensor
                (N, 8, 3) groundtruth bbx
            gt_label_tensor : torch.Tensor
                (N, ) groundtruth label.
        ...
        """
        # breakpoint()
        plt.figure(figsize=[(pc_range[3]-pc_range[0])/20, (pc_range[4]-pc_range[1])/20])
        pc_range = [int(i) for i in pc_range]
        pcd_np = pcd.cpu().numpy()

        # Define colors for each super class (prediction and ground truth)
        class_colors = {
            # Vehicle class
            1: {'pred': (255, 0, 0),      # Red 
                'gt': (0, 65, 0)},       # Dark green
            # Pedestrian class
            2: {'pred': (255, 128, 0),    # Orange
                'gt': (0, 255, 210)},     # Forest green
            # Cyclist class
            3: {'pred': (255, 0, 255),    # Magenta
                'gt': (0, 128, 255)},     # Light blue
            # Truck class
            # 4: {'pred': (255, 0, 128),    # Pink
            #     'gt': (0, 255, 0)}        # Bright green
        }
        ...
        pred_score_tensor = infer_result.get("pred_score", None)[:, 0]
        pred_label_tensor = infer_result.get("pred_score", None)[:, 1]
        legend_ax = plt.gca()
        legend_elements = []
        
        from matplotlib.patches import Patch
        for class_id, colors in class_colors.items():
            class_names = {1: 'Vehicle', 2: 'Pedestrian', 3: 'Truck'}
            legend_elements.extend([
                Patch(facecolor=np.array(colors['pred'])/255, 
                    label=f'{class_names[class_id]} (Pred)'),
                Patch(facecolor=np.array(colors['gt'])/255, 
        ...
        legend_ax.legend(handles=legend_elements,
                        ...
                        prop={'size': 6},      # Reduced font size from 8 to 6
                        handlelength=0.8,      # Reduced patch width from 1 to 0.8
                        handletextpad=0.3,     # Reduced space between patch and text from 0.5 to 0.3
                        labelspacing=0.2,      # Reduced vertical space between entries from 0.5 to 0.2
                        borderpad=0.2,         # Add smaller padding inside legend box
                        borderaxespad=0.2)     # Add smaller padding outside legend box
        if pred_box_tensor is not None:
            ...
            #     score_np = pred_score_tensor.cpu().numpy()
            #     pred_name = [f's:{score_np[i]:.3f}' for i in range(score_np.shape[0])]
        ...
        if pred_label_tensor is not None:
            pred_label_np = pred_label_tensor.cpu().numpy()
            pred_colors = np.array([class_colors[int(label)]['pred'] for label in pred_label_np])
        if gt_label_tensor is not None:
            gt_label_np = gt_label_tensor.cpu().numpy()
            gt_colors = np.array([class_colors[int(label)]['gt'] for label in gt_label_np])
        ...
        if method == 'bev':
            ...
            if gt_box_tensor is not None:
                canvas.draw_boxes(gt_box_np,colors=gt_colors, texts=gt_name)
                # canvas.draw_boxes(gt_box_np,colors=(0,255,0), texts=['']*len(gt_name), box_line_thickness=4) # paper visualization
            if pred_box_tensor is not None:
                canvas.draw_boxes(pred_box_np, colors=pred_colors, texts=pred_name)
...
def visualize_lidar_raw(pcd_list, pc_range, save_path, method='3d', left_hand=False):
    """
    Visualize raw LiDAR points from multiple agents with different colors.
    
    Parameters
    ----------
    pcd_list : list of torch.Tensor
        List of point clouds from different agents, each with shape (N, 4)
    pc_range : list
        [xmin, ymin, zmin, xmax, ymax, zmax]
    save_path : str
    ...
    agent_colors = [
        (0, 255, 0),    # Green for agent 1
        (0, 0, 0),      # Black for agent 2
        (255, 255, 0)   # Yellow for agent 3
    ]
    ...
    if method == 'bev':
        ...
        # Draw points for each agent with different colors
        for agent_idx, pcd in enumerate(pcd_list):
            pcd_np = pcd.cpu().numpy()
            canvas_xy, valid_mask = canvas.get_canvas_coords(pcd_np)
            # Create color array for all points of this agent
            point_colors = np.tile(agent_colors[agent_idx % len(agent_colors)], 
                                 (len(canvas_xy[valid_mask]), 1))
            canvas.draw_canvas_points(canvas_xy[valid_mask], colors=point_colors)
        ...
        raise ValueError(f"Visualization method {method} not supported.")
