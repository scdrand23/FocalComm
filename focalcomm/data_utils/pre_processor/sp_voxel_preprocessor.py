...
from focalcomm.data_utils.pre_processor.base_preprocessor import \
    BasePreprocessor
...
#         # Define central and outer regions
#         self.central_range = [-54.0, -40.0, -5.0, 54.0, 40.0, 3.0]
...
#         if self.spconv == 2:
#             self.central_generator = VoxelGenerator(
#                 vsize_xyz=[0.075, 0.075, 0.2],  # Fine resolution
#                 coors_range_xyz=self.central_range,
#                 max_num_points_per_voxel=self.max_points_per_voxel,
#                 num_point_features=4,
#                 max_num_voxels=self.max_voxels // 2
#             )
#             self.outer_generator = VoxelGenerator(
#                 vsize_xyz=self.voxel_size,
#                 coors_range_xyz=self.lidar_range,
#                 max_num_points_per_voxel=self.max_points_per_voxel,
#                 num_point_features=4,
...
#         # Split points into central and outer regions
#         central_mask = (pcd_np[:, 0] >= self.central_range[0]) & \
#                     (pcd_np[:, 0] <= self.central_range[3]) & \
#                     (pcd_np[:, 1] >= self.central_range[1]) & \
#                     (pcd_np[:, 1] <= self.central_range[4])
        
#         central_points = pcd_np[central_mask]
#         outer_points = pcd_np[~central_mask]
        
#         # Process both regions
#         central_pcd = tv.from_numpy(central_points)
#         outer_pcd = tv.from_numpy(outer_points)
        
#         central_output = self.central_generator.point_to_voxel(central_pcd)
#         outer_output = self.outer_generator.point_to_voxel(outer_pcd)
        
#         # Combine results
#         voxels = np.concatenate([central_output[0].numpy(), 
#                             outer_output[0].numpy()], axis=0)
#         coordinates = np.concatenate([central_output[1].numpy(),
#                                 outer_output[1].numpy()], axis=0)
#         num_points = np.concatenate([central_output[2].numpy(),
#                                 outer_output[2].numpy()], axis=0)
