...
from focalcomm.hypes_yaml import yaml_utils
...
def caluclate_tp_fp(det_boxes, det_score, gt_boxes, result_stat, iou_thresh):
    ...
    if det_boxes is not None:
        ...
        det_score = det_score[score_order_descend]  # sort scores from high to low
        ...
        if 'score' not in result_stat[iou_thresh]:
            result_stat[iou_thresh]['score'] = []
...
def calculate_ap(result_stat, iou):
    ...
    fp = iou_5['fp']
    tp = iou_5['tp']
    score = iou_5.get('score', [])
    
    assert len(fp) == len(tp)
    
    # Sort by score if available
    if score and len(score) == len(fp):
        # Sort in descending order of score
        sorted_indices = np.argsort(-np.array(score))
        fp = [fp[i] for i in sorted_indices]
        tp = [tp[i] for i in sorted_indices]
